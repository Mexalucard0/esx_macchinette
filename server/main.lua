...
function LoadShop()
	...
	for i=1, #shopResult, 1 do
		...
		table.insert(ShopItems[shopResult[i].store], {
			...
			item  = shopResult[i].item,
			price = shopResult[i].price,
			weight = itemInformation[shopResult[i].item].weight
		})
	end
end
...
ESX.RegisterServerCallback('esx_shops:requestDBItems', function(source, cb)
	if not hasSqlRun then
		TriggerClientEvent('esx:showNotification', source, 'Il database del negozio non è stato ancora caricato, riprova tra qualche istante.')
	end
